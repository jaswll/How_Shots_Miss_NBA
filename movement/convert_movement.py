...
if __name__ == "__main__":
    ...
    # csv_dir = 'C:/Users/Jason/Documents/Capstone/stage1/data/test_csv'
    # converted_dir = 'C:/Users/Jason/Documents/Capstone/stage1/data/test_converted'
    if not os.path.exists(converted_dir):
        os.makedirs(converted_dir)

    games = os.listdir(csv_dir)
    converted_games = os.listdir(converted_dir)
    ...
    for game in tqdm(games):
        ...
        if converted_game in converted_games:
            ...
            data = pd.read_csv('%s/%s.csv' % (csv_dir, str(game)), compression='gzip')
            data = full_to_half_full(data)
            data = half_full_to_half(data)
            data.to_csv('%s/%s_converted.csv' % (converted_dir, str(game)), index=False, compression='gzip')
